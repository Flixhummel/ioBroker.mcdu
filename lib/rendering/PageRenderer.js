...
class PageRenderer {
    ...
    async renderPage(pageId) {
        try {
            ...
            // Pagination: collect all interactive items (lines with display content)
            const oddRows = [1, 3, 5, 7, 9, 11];
            const allOddItems = pageConfig.lines?.filter(l => l.display && l.display.type !== 'empty') || [];
            ...
            // Build paginated line map: display row → line config
            ...
            if (this.totalPages > 1) {
                ...
                pageItems.forEach((item, i) => {
                    const displayRow = oddRows[i];
                    paginatedMap.set(displayRow, item);
                });
            }
            ...
            for (let row = 1; row <= this.rows; row++) {
                try {
                    // Even rows (2,4,6,8,10): render sub-label for the NEXT odd row
                    if (row % 2 === 0 && row <= 10) {
                        const nextOddRow = row + 1;
                        const nextLineConfig = this.totalPages > 1
                            ? paginatedMap.get(nextOddRow)
                            : pageConfig.lines?.find(l => l.row === nextOddRow);
                        const subLabel = nextLineConfig?.subLabel || '';
                        lines.push({
                            text: this.padOrTruncate(subLabel ? ` ${subLabel}` : '', this.columns),
                            color: 'cyan'
                        });
                        continue;
                    }
                    ...
                    const lineConfig = this.totalPages > 1 && oddRows.includes(row)
                        ...
                        : pageConfig.lines?.find(l => l.row === row);
    ...
    renderStatusBar(pageId) {
        ...
        // Build status bar: left-align name, right-align page indicator + time

...
function flattenPages(pages) {
    ...
    return pages.map(page => {
        // Migrate old row numbers (1,3,5→3,5,7) before flattening to slots
        const migrated = Array.isArray(page.lines) ? migrateOldRows(page.lines) : [];
        return {
            ...page,
            lines: migrated.map(flattenLine)
        };
    });
}

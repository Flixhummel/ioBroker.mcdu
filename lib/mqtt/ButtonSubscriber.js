...
class ButtonSubscriber {
    ...
    async handleFunctionKey(button) {
        ...
        switch (button) {
            case 'MENU':
                // Always go to home page (first page)
                this.adapter.log.info('MENU key - navigating to home page');
                await this.adapter.navigateHome();
                break;

            case 'INIT':
                // Navigate to status page (if exists) or first page with id containing 'status'
                this.adapter.log.info('INIT key - navigating to status page');
                {
                    const statusPage = this.adapter.config.pages?.find(
                        p => p.id === 'status-main' || p.id.includes('status')
                    );
                    if (statusPage) {
                        await this.adapter.switchToPage(statusPage.id);
                    } else {
                        this.adapter.log.debug('No status page configured');
                    }
                }
                break;

            case 'DIR':
                // Direct page access: if scratchpad has content, try navigating to that page ID
                this.adapter.log.info('DIR key - direct page access');
                if (this.inputModeManager) {
                    ...
                    if (content) {
                        ...
                        this.adapter.log.debug('DIR pressed with empty scratchpad');
                    }
                }
                break;

            case 'PREV_PAGE':
                // Paginate: go to previous sub-page
            ...
            case 'FPLN':
                // Favorites / frequently used pages
                this.adapter.log.info('FPLN key - favorites');
                {
                    const favPage = this.adapter.config.pages?.find(
                        p => p.id === 'scenes-main' || p.id.includes('scene') || p.id.includes('favorites')
                    );
                    if (favPage) {
                        await this.adapter.switchToPage(favPage.id);
                    }
                }
                break;
            ...
            case 'PERF':
                ...
                this.adapter.log.info('PERF key - performance');
                {
                    const perfPage = this.adapter.config.pages?.find(
                        p => p.id.includes('perf') || p.id.includes('stats')
                    );
                    if (perfPage) {
                        await this.adapter.switchToPage(perfPage.id);
                    }
                }
                break;

            default:
                this.adapter.log.debug(`Function key ${button} not handled`);
                break;
        }
    }
